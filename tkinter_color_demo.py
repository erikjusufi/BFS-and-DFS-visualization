...
class App(Frame):
    ...
    def potvrdi_dimenzije(self):
        '''
        Funkcija se poziva pritiskom na gumb "Potvrdi" i inicijalizira matricu
        '''
    ...
    def promijeni_boju(self, event):
        '''
        Funkcija definira BFS pretragu i mijenja boju elementa matrice i svih susjednih elemenata dok ne naiđe na rub matrice ili na znak #
        '''
        ...
        # nova boja iz funkcije random color
        boja = self.random_color()
        # promijeni boju elementa
        self.matrica[i,j].configure(bg=boja)
        
        # dodaj inicijalni element u red i u skup posjećenih elemenata
        queue.put((i,j))
        visited.add((i,j))

        # BFS pretraga
        # dok red nije prazan, dohvati element iz reda i provjeri susjedne elemente, ako nisu u skupu posjećenih elemenata, dodaj ih u red i u skup posjećenih elemenata
        while not queue.empty():
            i, j = queue.get()
            if j != 0 and (i,j-1) not in visited:
                if self.matrica[i,j-1]["text"] != "#":
                    self.matrica[i,j-1].configure(bg=boja)
                    queue.put((i,j-1))
                    visited.add((i,j-1))
            ...
            if i != 0 and (i-1,j) not in visited:
                if self.matrica[i-1,j]["text"] != "#":
                    self.matrica[i-1,j].configure(bg=boja)
                    queue.put((i-1,j))
                    visited.add((i-1,j))
    ...
    def random_color(self):
        '''
        Funkcija generira nasumičnu boju u hex zapisu
        '''
        color = "#"
        for i in range(6):
