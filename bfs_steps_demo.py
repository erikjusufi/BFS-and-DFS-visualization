...
class App(Frame):
    ...
    def distance_to(self, event):
        ...
        self.matrix[i,j].configure(bg=color, fg="white", text="0")
        
        # add initial element to queue and set of visited elements, with it add number of steps to next element
        queue.put((i,j,1))
        visited.add((i,j))
        ...
        while not queue.empty():
            ...
            # get element from queue with number of steps to its neighbors
            i, j, steps = queue.get()
            time.sleep(float(self.delay.get()))
            if j != 0 and (i,j-1) not in visited:
                if self.matrix[i,j-1]["text"] != "#":
                    # change text of element to steps to it
                    self.matrix[i,j-1].configure(bg=color, fg="white", text=steps)
                    # add new element to queue and set of visited elements with number of steps to it
                    queue.put((i,j-1, steps+1))
                    visited.add((i,j-1))
